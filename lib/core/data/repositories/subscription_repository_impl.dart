...
class SubscriptionRepositoryImpl implements SubscriptionRepository {
  ...
  bool get _useMockPurchases =>
      ...
      AppConstants.revenueCatApiKey.trim().isEmpty;
  ...
  Future<SubscriptionModel?> getUserSubscription(String userId) =>
      _dataSource.getSubscription(userId);

  @override
  Future<bool> isSubscribed(String userId) async =>
      (await getUserSubscription(userId))?.isActive ?? false;
  ...
  Future<SubscriptionModel> _activateMockPremium(
          {required String userId, required String plan}) =>
      _saveActiveSubscription(userId: userId, plan: plan);

  Future<SubscriptionModel> _saveActiveSubscription(
      {required String userId, required String plan}) async {
    final now = DateTime.now().toUtc();
    final expiresAt = plan == 'premium_lifetime'
        ? null
        : now.add(plan == 'premium_monthly'
            ? const Duration(days: 30)
            : const Duration(days: 365));
    await _dataSource.saveSubscription({
      'user_id': userId,
      'plan': plan,
      'status': 'active',
